...
class HookRunner:
    def __init__(
        self, root: Path, base_env: dict[str, str], retries: int, logger: logging.Logger
    ) -> None:
        self._root = root
...
def load_workflow(path: Path) -> WorkflowConfig:
    ...
    return WorkflowConfig(
        phases=phases, environment=environment, retries=retries, init=init
    )
...
def _parse_transitions(
    phase_name: str, raw_phase: dict[str, Any]
